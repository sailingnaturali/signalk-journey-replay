...
function rewrite(delta, meta, offset, mode) {
  ...
  out.updates = delta.updates.map(u => {
    const base = mode === 'rebase'
      ? { ...u, timestamp: new Date(Date.parse(u.timestamp) + offset).toISOString() }
      : { ...u }
    const originalSource = u.$source || (u.source && u.source.label) || 'unknown'
    base.$source = 'journey-replay.' + originalSource
    delete base.source
    return base
  })
  return out
